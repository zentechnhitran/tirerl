...
make_request({count, Index, _Type, Doc, Params}) ->
	IndexList = join(Index, <<", ">>),
	%TypeList = join(Type, <<", ">>),
	Uri = make_uri([IndexList, ?COUNT], Params),
	#{method => get, uri => Uri, body => Doc};

make_request({delete_by_query, Index, _Type, Doc, Params}) ->
	IndexList = join(Index, <<", ">>),
	%TypeList = join(Type, <<", ">>),
	Uri = make_uri([IndexList, <<"_doc">>, ?DELETE_BY_QUERY], Params),
...
make_request({insert_doc, Index, _Type, Id, Doc, Params}) ->
	#{method => put,
		uri => make_uri([Index, <<"_doc">>, Id], Params),
		body => Doc};
...
make_request({delete_doc, Index, _Type, Id, Params}) ->
	#{method => delete, uri => make_uri([Index, <<"_doc">>, Id], Params)};

make_request({search, Index, _Type, Doc, Params}) ->
	Uri = make_uri([Index, ?SEARCH], Params),
	#{method => get,
		uri => Uri,
		body => Doc};
